...
**NEXUS** (Network of EXperts, Unified in Strategy) turns The Agency's 人工智能 specialists into a coordinated pipeline. Instead of activating agents one at a time and hoping they work together, NEXUS defines exactly who does what, when, and how quality is verified at every step.
...
- Phase 0: Discovery (Trend Researcher, Feedback Synthesizer, 用户体验研究er, Analytics Reporter, 法律合规 Checker, Tool Evaluator)
...
- Phase 4: Harden (Reality Checker, Performance Benchmarker, API Tester, 法律合规 Checker)
- Phase 5: Launch (增长 Hacker, Content Creator, all marketing agents, DevOps Automator)
...
Then activate Evidence Collector to confirm no visual r出口ions.
```

### Run a Marketing Campaign
```
Activate Social Media Strategist as campaign lead for [CAMP人工智能GN DESCRIPTION].
...
### Market Research
```
Activate Trend Researcher for market intelligence on [DOM人工智能N].
交付物: Competitive landscape, market sizing, trend forecast.
After research, activate 执行摘要 Generator for executive brief.
```

### UX Improvement
```
Activate 用户体验研究er to identify usability issues in [FEATURE/PRODUCT].
...
| **Phase 4 Playbook** | Quality & 加固 | `strategy/playbooks/phase-4-加固.md` |
...
| **交接 Templates** | Standardized 交接 formats | `strategy/coordination/交接-templates.md` |
| **Startup MVP Runbook** | 4-6 week MVP build | `strategy/运行手册/scenario-startup-mvp.md` |
| **Enterprise Feature Runbook** | Enterprise feature development | `strategy/运行手册/scenario-enterprise-feature.md` |
| **Marketing Campaign Runbook** | Multi-channel campaign | `strategy/运行手册/scenario-marketing-campaign.md` |
| **事件响应 Runbook** | Production incident 处理 | `strategy/运行手册/scenario-incident-response.md` |
...
2. **Dev↔QA Loop** — Every task is built then tested; PASS to proceed, F人工智能L to retry (max 3)
3. **交接s** — Structured context transfer between agents (never start cold)
4. **Reality Checker** — Final quality authority; defaults to "NEEDS WORK"
5. **Agents Orchestrator** — Pipeline controller 管理 the entire flow
...
Frontend Developer  │ 界面设计er         │ 增长 Hacker
Backend Architect   │ 用户体验研究er       │ Content Creator
Mobile App Builder  │ UX Architect        │ Twitter Engager
人工智能 Engineer         │ Brand Guardian      │ TikTok Strategist
...
Feedback Synthesizer│ Studio Operations   │ Test 结果分析器
...
法律合规    │ visionOS Spatial    │ Data Consolidation
Exec 总结 Gen.   │ Terminal Integration│ Report Distribution
```
