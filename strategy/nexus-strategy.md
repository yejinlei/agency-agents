...
| **Context Continuity** | Every 交接 carries full context — no agent starts cold |
| **Parallel Execution** | Independent workstreams run concurrently to compress 时间线s |
...
|----------|--------|--------------------|
| **工程** | Frontend Developer, Backend Architect, Mobile App Builder, 人工智能 Engineer, DevOps Automator, Rapid Prototyper, Senior Developer | Build, deploy, and maintain all technical systems |
| **Design** | 界面设计er, 用户体验研究er, UX Architect, Brand Guardian, Visual Storyteller, Whimsy Injector, Image Prompt Engineer | Define visual identity, 用户体验, and brand consistency |
| **Marketing** | 增长 Hacker, Content Creator, Twitter Engager, TikTok Strategist, Instagram Curator, Reddit Community Builder, App Store Optimizer, Social Media Strategist | Drive acquisition, engagement, and market presence |
| **Product** | Sprint Prioritizer, Trend Researcher, Feedback Synthesizer | Define what to build, when, and why |
| **Project Management** | Studio Producer, Project Shepherd, Studio Operations, Experiment Tracker, Senior Project Manager | Orchestrate 时间线s, resources, and 跨职能 coordination |
| **测试** | Evidence Collector, Reality Checker, Test 结果分析器, Performance Benchmarker, API Tester, Tool Evaluator, Workflow Optimizer | Verify quality through evidence-based assessment |
| **Support** | Support Responder, Analytics Reporter, Finance Tracker, Infrastructure Maintainer, 法律合规 Checker, 执行摘要 Generator | Sustain operations, compliance, and business intelligence |
...
| **Specialized** | Agents Orchestrator, Analytics Reporter, LSP/Index Engineer, Sales Data Extraction Agent, Data Consolidation Agent, Report Distribution Agent | Cross-剪切 coordination, deep analytics, and code intelligence |
...
│  Intelligence   架构    Foundation       Dev ↔ QA Loop          │
│                                                                         │
...
```
     ...
     │  工程 │ Design │ Marketing │ Product │ QA │
     └─────────────────────────────────────────────────┘
```

### 2.3 Activation Modes

NEXUS supports three 部署 configurations:

| Mode | Agents Active | Use Case | 时间线 |
|------|--------------|----------|----------|
...
> **目标**: Understand the landscape before committing resources. No 构建 until the problem is validated.

### 3.1 Active Agents

| Agent | Role in Phase | Primary Output |
|-------|--------------|----------------|
| **Trend Researcher** | Market intelligence lead | 市场分析 Report with TAM/SAM/SOM |
| **Feedback Synthesizer** | User needs analysis | Synthesized Feedback Report with pain points |
| **用户体验研究er** | User behavior analysis | Research Findings with personas and 旅程图s |
| **Analytics Reporter** | Data landscape assessment | Data Audit Report with available signals |
| **法律合规 Checker** | Regulatory scan | Compliance 要求 Matrix |
...
| User need confirmed | ≥3 validated pain points | Feedback Synthesizer + 用户体验研究er data |
| Regulatory path clear | No blocking compliance issues | 法律合规 Checker matrix |
...
**Output**: 执行摘要 (≤500 words, SCQA format) → Decision: GO / NO-GO / PIVOT

---

## 4. Phase 1 — Strategy & 架构

> **目标**: Define what we're 构建, how it's structured, and what success looks like — before 编写 a single line of code.
...
| **Sprint Prioritizer** | Feature 优先级排序 | Prioritized Backlog (RICE scored) |
| **UX Architect** | Technical architecture + UX foundation | 架构 Spec + CSS Design System |
| **Brand Guardian** | Brand identity system | Brand Foundation Document |
| **Backend Architect** | System architecture | System 架构 Specification |
| **人工智能 Engineer** | 人工智能/ML architecture (if applicable) | ML 系统设计 |
| **Finance Tracker** | Budget and resource 规划 | Financial Plan with ROI projections |
...
STEP 2: Technical 架构 (Parallel, after Step 1)
├── UX Architect → CSS Design System + Layout Framework + UX Structure
├── Backend Architect → System 架构 (服务s, databases, APIs)
├── 人工智能 Engineer → ML 架构 (models, pipelines, 推理 strategy)
...
└── Sprint Prioritizer → RICE-scored backlog with sprint assignments
    ├── Input: Task List + 架构 Spec + Budget Framework
...
**关卡守门人**: Studio Producer + Reality Checker (dual 签核)

| Criterion | Threshold | Evidence Required |
|-----------|-----------|-------------------|
| 架构 covers all requirements | 100% spec coverage | Senior PM task list cross-referenced |
...
**Output**: Approved 架构 Package → Phase 2 activation

---

## 5. Phase 2 — Foundation & Scaffolding

> **目标**: Build the technical and operational foundation that all subsequent work depends on. Get the skeleton standing before 添加 muscle.

### 5.1 Active Agents

| Agent | Role in Phase | Primary Output |
|-------|--------------|----------------|
| **DevOps Automator** | 持续集成/持续部署 pipeline + infrastructure | Deployment Pipeline + IaC Templates |
...
| **Infrastructure Maintainer** | Cloud infrastructure setup | Monitoring + Logging + 告警 |
| **Studio Operations** | Process setup | Collaboration tools + 工作流程 |
...
│   ├── 持续集成/持续部署 pipeline (GitHub Actions)   │   ├── Project scaffolding
...
│   ├── Cloud resource provisioning       │   ├── Database schema 部署
│   ├── Monitoring (Prometheus/Grafana)   │   ├── API scaffold + auth
│   └── 安全 加固               │   └── Service communication layer
│                                         │
└── Studio Operations                     └── UX Architect
    ├── Git 工作流程 + branch strategy        ├── CSS design tokens
    ├── 沟通 channels                ├── Responsive layout system
...
| Criterion | Threshold | Evidence Required |
|-----------|-----------|-------------------|
| 持续集成/持续部署 pipeline operational | Build + test + deploy working | Pipeline execution logs |
| Database schema deployed | All tables/indexes created | Migration success + schema dump |
| API scaffold 响应 | Health check endpoints live | curl response screenshots |
| Frontend 渲染 | Skeleton app loads in browser | Evidence Collector screenshots |
| Monitoring active | 仪表板s 显示 metrics | Grafana/监控 screenshots |
...
> **目标**: Implement features through continuous Dev↔QA loops. Every task is validated before the next begins. This is where the bulk of the work happens.
...
|-----------|------------------|----------|-------------------|
| Frontend UI | Frontend Developer | Evidence Collector | 界面设计er, Whimsy Injector |
| Backend API | Backend Architect | API Tester | Performance Benchmarker |
| Database | Backend Architect | API Tester | Analytics Reporter |
| Mobile | Mobile App Builder | Evidence Collector | 用户体验研究er |
| 人工智能/ML Feature | 人工智能 Engineer | Test 结果分析器 | Analytics Reporter |
...
| Code Intelligence | LSP/Index Engineer | Test 结果分析器 | Senior Developer |
...
```
TRACK A: Core Product                    TRACK B: 增长 & Marketing
├── Frontend Developer                   ├── 增长 Hacker
...
├── 人工智能 Engineer                          ├── Social Media Strategist
...
│   └── Load 测试 + optimization      │   └── Visual narrative assets
...
| API 端点 validated | All endpoints tested | API Tester report |
...
| No critical bugs | Zero P0/P1 open issues | Test 结果分析器 summary |
...
| Agent | Role in Phase | Primary Output |
|-------|--------------|----------------|
| **Reality Checker** | Final 集成测试 (defaults to NEEDS WORK) | Reality-Based Integration Report |
| **Evidence Collector** | Comprehensive visual evidence | Screenshot Evidence Package |
| **Performance Benchmarker** | Load 测试 + optimization | Performance Certification |
| **API Tester** | Full API r出口ion suite | API Test Report |
| **Test 结果分析器** | Aggregate quality metrics | Quality 指标 仪表板 |
| **法律合规 Checker** | Final compliance audit | Compliance Certification |
...
├── API Tester → Complete endpoint r出口ion
├── Performance Benchmarker → Load test at 10x expected traffic
└── 法律合规 Checker → Final regulatory audit

STEP 2: Analysis (Parallel, after Step 1)
├── Test 结果分析器 → Aggregate all 测试数据 into quality dashboard
...
└── Reality Checker → Integration Report
    ├── Cross-validates ALL previous QA 查找s
    ├── Tests complete 用户旅程s with screenshot evidence
...
**关卡守门人**: Reality Checker (sole authority)
...
| Performance certified | P95 < 200ms, 正常运行时间 > 99.9% | Load test results |
| 安全 validated | Zero critical vulnerabilities | 安全 scan report |
| Compliance certified | All regulatory requirements met | 法律合规 Checker report |
...
> **目标**: Coordinate the go-to-market execution across all channels simultaneously. Maximum impact at launch.

### 8.1 Active Agents

| Agent | Role in Phase | Primary Output |
|-------|--------------|----------------|
| **增长 Hacker** | Launch strategy lead | 增长 Playbook with viral loops |
...
| **执行摘要 Generator** | Stakeholder communication | Launch 执行摘要 |
| **Project Shepherd** | Launch coordination | Launch Checklist + 时间线 |
| **DevOps Automator** | Deployment execution | Zero-停机时间 部署 |
| **Infrastructure Maintainer** | Launch 监控 | Real-time dashboards |
...
├── 增长 Hacker → Viral mechanics tested and armed
├── App Store Optimizer → Store listing optimized
├── DevOps Automator → Blue-green 部署 prepared
└── Infrastructure Maintainer → Auto-扩展 configured for 10x

T-0: Launch Day
├── DevOps Automator → Execute 部署
...
├── 增长 Hacker → Analyze acquisition data, optimize funnels
...
└── 执行摘要 Generator → Daily stakeholder briefings
```
...
| Criterion | Threshold | Evidence Required |
|-----------|-----------|-------------------|
| Deployment successful | Zero-停机时间, all health checks pass | DevOps 部署 logs |
| Systems stable | No P0/P1 incidents in first 48 hours | Infrastructure 监控 |
| User acquisition active | Channels driving traffic | Analytics Reporter dashboard |
| Feedback loop operational | User feedback 是 collected | Feedback Synthesizer report |
| 干系人 informed | Executive summary delivered | 执行摘要 Generator output |
...
> **目标**: Sustained operations with continuous improvement. The product is live — now make it thrive.

### 9.1 Active Agents (Ongoing)

| Agent | Cadence | Responsibility |
|-------|---------|---------------|
| **Infrastructure Maintainer** | Continuous | System reliability, 正常运行时间, performance |
| **Support Responder** | Continuous | Customer support and issue resolution |
| **Analytics Reporter** | Weekly | KPI 追踪, dashboards, insights |
| **Feedback Synthesizer** | Bi-weekly | User feedback analysis and synthesis |
| **Finance Tracker** | Monthly | Financial performance, budget 追踪 |
| **法律合规 Checker** | Monthly | Regulatory 监控 and compliance |
| **Trend Researcher** | Monthly | Market intelligence and competitive analysis |
| **执行摘要 Generator** | Monthly | C-suite 报告 |
| **Sprint Prioritizer** | Per sprint | Backlog grooming and 冲刺规划 |
| **Experiment Tracker** | Per experiment | A/B test management and analysis |
| **增长 Hacker** | Ongoing | Acquisition optimization and growth experiments |
...
│       └────────────────────────────────────────────┘      │
│                                                           │
│  Monthly: 执行摘要 Generator → C-suite report    │
│  Monthly: Finance Tracker → Financial performance         │
│  Monthly: 法律合规 Checker → Regulatory update    │
...
### 10.2 Critical 交接 Pairs

These are the highest-traffic 交接 relationships in NEXUS:
...
| Evidence Collector | Agents Orchestrator | QA Verdict (PASS/F人工智能L) | Per task |
...
| 执行摘要 Generator | Studio Producer | Executive Brief | Per milestone |

---

## 11. 交接协议s

### 11.1 Standard 交接 Template

Every agent-to-agent 交接 must include:
...
- **交接 to next**: [Who receives the output and what they need]
```
...
### 升级 Level: [To Agents Orchestrator / To Studio Producer]

### Failure History
- Attempt 1: [总结 of issues and fixes attempted]
- Attempt 2: [总结 of issues and fixes attempted]
- Attempt 3: [总结 of issues and fixes attempted]
...
| Phase | Gate Name | 关卡守门人 | Pass Criteria |
|-------|-----------|-------------|---------------|
| 0 → 1 | Discovery Gate | 执行摘要 Generator | Market validated, user need confirmed, regulatory path clear |
| 1 → 2 | 架构 Gate | Studio Producer + Reality Checker | 架构 complete, brand defined, budget approved, sprint plan realistic |
| 2 → 3 | Foundation Gate | DevOps Automator + Evidence Collector | 持续集成/持续部署 working, skeleton app running, 监控 active |
...
```
IF gate F人工智能LS:
  ├── 关卡守门人 produces specific failure report
...
| 安全 Vulnerability | 法律合规 Checker | Infrastructure Maintainer | DevOps Automator |
| Performance Degradation | Performance Benchmarker | Infrastructure Maintainer | Backend Architect |
| Brand Inconsistency | Brand Guardian | 界面设计er | Studio Producer |
...
| Regulatory Non-Compliance | 法律合规 Checker | Support Responder | Studio Producer |
| Market Shift | Trend Researcher | 增长 Hacker | Studio Producer |
...
| System 正常运行时间 | > 99.9% | Infrastructure Maintainer |
| Lighthouse score | > 90 (Performance + 无障碍) | Frontend Developer |
| 安全 vulnerabilities | Zero critical | 法律合规 Checker |
...
| Metric | Target | Measurement Agent |
|--------|--------|-------------------|
| User acquisition (MoM) | 20%+ growth | 增长 Hacker |
...
| Compliance adherence | 98%+ | 法律合规 Checker |
| Stakeholder satisfaction | 4.5/5 | 执行摘要 Generator |
...
#
# Compliance Audit:
#   法律合规 Checker → 执行摘要 Generator
#
# Market Research:
#   Trend Researcher → Analytics Reporter → 执行摘要 Generator
#
# UX Improvement:
#   用户体验研究er → UX Architect → Frontend Developer → Evidence Collector
```
...
```
你是一个 the Agents Orchestrator running NEXUS pipeline for [PROJECT].
...
3. Manage 交接 using the NEXUS 交接 Template
4. Enforce quality gates before phase advancement
5. Track all tasks with status 报告
...
```
你是一个 [AGENT NAME] working within the NEXUS pipeline.

阶段： [Current Phase]
任务： [Task ID and description from Sprint Prioritizer backlog]
架构 reference: [path to architecture doc]
...
```
你是一个 [QA AGENT] 验证 work within the NEXUS pipeline.
...
2. 架构 specification: [path]
3. Brand guidelines: [path]
4. Performance requirements: [specific thresholds]

Provide verdict: PASS or F人工智能L
If F人工智能L: Include specific issues, evidence, and fix instructions
...
### 工程 Division — "Build It Right"
| Agent | Superpower | 激活触发条件 |
|-------|-----------|-------------------|
...
| 人工智能 Engineer | ML models, LLMs, 检索增强生成 systems, 数据管道 | Any 人工智能/ML feature |
| DevOps Automator | 持续集成/持续部署, IaC, Kubernetes, 监控 | Infrastructure or 部署 work |
...
| Agent | Superpower | 激活触发条件 |
|-------|-----------|-------------------|
| 界面设计er | Visual design systems, component libraries | Interface design or component creation |
| 用户体验研究er | User 测试, behavior analysis, personas | User research or 可用性测试 |
...
| Image Prompt Engineer | 人工智能 图像生成 prompts, photography | Photography prompt creation for 人工智能 tools |

### Marketing Division — "Grow It Fast"
| Agent | Superpower | 激活触发条件 |
|-------|-----------|-------------------|
| 增长 Hacker | Viral loops, funnel optimization, experiments | User acquisition or growth strategy |
...
| Agent | Superpower | 激活触发条件 |
|-------|-----------|-------------------|
| Sprint Prioritizer | RICE scoring, agile 规划, velocity | Sprint 规划 or 待办梳理 |
| Trend Researcher | Market intelligence, competitive analysis | Market research or opportunity assessment |
| Feedback Synthesizer | User feedback analysis, 情感分析 | User feedback processing |

### Project Management Division — "Keep It on Track"
| Agent | Superpower | 激活触发条件 |
|-------|-----------|-------------------|
| Studio Producer | Portfolio strategy, executive orchestration | Strategic 规划 or portfolio management |
...
| Experiment Tracker | A/B 测试, hypothesis validation | Experiment management |
| Senior Project Manager | Spec-to-task conversion, realistic scoping | Task 规划 or scope management |

### 测试 Division — "Prove It Works"
| Agent | Superpower | 激活触发条件 |
|-------|-----------|-------------------|
| Evidence Collector | Screenshot-based QA, visual proof | Any visual verification need |
| Reality Checker | Evidence-based certification, skeptical assessment | Final 集成测试 |
| Test 结果分析器 | Test evaluation, quality metrics | Test output analysis |
| Performance Benchmarker | Load 测试, performance optimization | Performance 测试 |
| API Tester | API validation, 集成测试 | API 端点 测试 |
...
| Agent | Superpower | 激活触发条件 |
|-------|-----------|-------------------|
| Support Responder | Customer 服务, issue resolution | Customer support needs |
| Analytics Reporter | Data analysis, dashboards, KPI 追踪 | Business intelligence or 报告 |
| Finance Tracker | Financial 规划, budget management | Financial analysis or budgeting |
| Infrastructure Maintainer | System reliability, performance optimization | Infrastructure management |
| 法律合规 Checker | Compliance, regulations, legal review | Legal or compliance needs |
| 执行摘要 Generator | C-suite communication, SCQA framework | Executive 报告 |

### Spatial Computing Division — "Immerse Them"
| Agent | Superpower | 激活触发条件 |
|-------|-----------|-------------------|
| XR Interface Architect | Spatial 交互设计 | AR/VR/XR interface design |
...
| Terminal Integration Specialist | CLI tools, terminal 工作流程 | Developer tool integration |

### Specialized Division — "Connect Everything"
| Agent | Superpower | 激活触发条件 |
|-------|-----------|-------------------|
| Agents Orchestrator | Multi-agent pipeline management | Any multi-agent 工作流程 |
...
| Sales Data Extraction Agent | Excel 监控, sales metric extraction | Sales data ingestion |
| Data Consolidation Agent | Sales data aggregation, dashboard reports | Territory and rep 报告 |
| Report Distribution Agent | Automated report delivery | 时间表d report distribution |
...
|------|-----------|
| **NEXUS** | Network of EXperts, Unified in Strategy |
| **Quality Gate** | Mandatory 检查点 between phases requiring evidence-based approval |
| **Dev↔QA Loop** | Continuous development-测试 cycle where each task must pass QA before proceeding |
| **交接** | Structured transfer of work and context between agents |
| **关卡守门人** | Agent(s) with authority to approve or reject phase advancement |
| **升级** | Routing a blocked task to higher authority after retry exhaustion |
...
| **Context Continuity** | Principle that every 交接 carries full context |
...
*From discovery to sustained operations — every agent knows their 角色, their timing, and their 交接.*
