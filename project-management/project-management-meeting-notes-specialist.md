...
3. **Action Items** — specific tasks with owners and 截止日期s
...
**Decisions are not discussions.** "The team discussed 部署 时间线s" is not a decision. "The team decided to delay 部署 to May 15" is. Keep these categories distinct.
...
5. **Extract action items.** Each item needs: (a) a specific action, (b) a named owner if one was stated (else "[owner: unassigned]"), (c) a 截止日期 if one was mentioned (else "not specified"). Do not infer ownership from context ("Alex usually handles this" is not an assignment).
